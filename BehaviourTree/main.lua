...
local node = BT.WaitNode:create(function() return math.random() * 0.2 + 0.2 end)
...
local lock_time1 = os.clock() - start1

local start2 = os.clock()
local node = BT.WaitNode:create(function() return math.random() * 0.5 + 0.5 end)
while node.status == RUNNING or node.status == READY do
	node:Visit()
end
local lock_time2 = os.clock() - start2


print("Test Ended \t" .. testtype .. "\n")
assert(lock_time1 >= 0.2, "BT.WaitNode WaitTime Must Big Than 0.2")
assert(lock_time1 <= 0.4, "BT.WaitNode WaitTime Must Small Than 0.4")
assert(lock_time2 >= 0.5, "BT.WaitNode WaitTime Must Big Than 0.5")
assert(lock_time2 <= 1, "BT.WaitNode WaitTime Must Small Than 1")
...
for i = 1, 10 do
	local random_loop_count = nil
	local node = BT.LoopNode:create({node_action_add1, node_action_add1}, function()
		random_loop_count = math.random(5, 9)
		return random_loop_count
	end)
	...
	_should_time = _should_time + 2 * random_loop_count
	assert(_time == _should_time, "BT.LoopNode No Work")
end
...
testtype = "BT.RandomNode weights"
print("Test Start\t" .. testtype)
...
assert(node._weights[2] == 0, "RandomNode weights[2] must == 0")
assert(node._weights[3] == 0, "RandomNode weights[3] must == 0")
...
testtype = "BT.RandomNode function weights"
print("Test Start\t" .. testtype)
...
local node = BT.RandomNode:create({
		...
		function() return math.random(3) + 0 end,
		function() return math.random(3) + 3 end,
		function() return math.random(3) + 6 end,
	})
...
local sum = random_1 + random_2 + random_3

print(sum, random_1, random_2, random_3)
print(random_1 / sum, random_2 / sum, random_3 / sum)
